...
export function startAppManager(component) {
  ...
  window.onload = function () {
    ...
    setEvent("mousedown", (e) => {
      ...
      const playerClan = Array.from(_scene.clans).find(
        (c) => c.player === TYPE.PLAYER
      );
  ...
  function updateUnit() {
    ...
    Array.from(_scene.clans).forEach((_clan) => {
      Array.from(_clan.units).forEach((_unit) => {
        ...
        if (_unit.angle < _unit.destAngle) _unit.angle += 5;
        else _unit.angle -= 5;
  ...
  function ShowFramePerSec() {
    const sample = setInterval(() => {
      frameRate.set(framesPerSec);
      framesPerSec=0;
      console.log("frameRate:", frameRate.get());
